...
class Segment_Commerce_Woo extends Segment_Commerce {
        ...
        if ( did_action( 'woocommerce_thankyou' ) ) {
            ...
            if ( 'failed' !== $order->status ) {
                ...
                foreach ( $items as $item ) {
                    ...
                    $product = array(
                        'id'       => $item['product_id'],
                        'sku'      => (!is_null($_product)) ? $_product->get_sku() : '',
                        'name'     => $item['name'],
     ...
     *
     */
    // TODO: Make sure this is correct (it's a suggested replacement for deprecated $item->get_product_from_item())
    protected function get_product_from_item(WC_Order_item $item)
    {
